...
read -p "Which remote directory would you like to use? (full path (example: '/var/www/html/yoursite/') " remotedir
read -p "Please select the file to be deleted (from the remote host (full path (example: '/var/www/html/yoursite/file.html')): " delfile
...
ssh -p $rhostport $user@$ip rm -rf $delfile
rm /root/scripts/SECSUITE/LEROY/qry1.txt
echo "File has been deleted. List of files in $remotedir:"
ssh -p $rhostport $user@$ip ls -la $remotedir
exit
