...
if grep -q /root/scripts/SECSUITE/inframon/apachestatus/apache-monitor-$hostname.sh "apachemonitorcheck.txt"; then
    ...
    rm apachemonitorcheck.txt $constructfile
  else
...
echo ''
#Remove sources
rm $installscript optionfile.txt final-ips.txt nmapfile.txt nmapfiltered.txt sanitised.txt ipaddrfile.txt inetfile.txt netfile.txt $constructfile
#
...
if ! grep -q loggedusers "dbfile.txt"; then
        ...
        mysql -h $ipaddr -D'status' --user="$mysqluser" --password="$mysqlpass" -e "CREATE TABLE loggedusers (hostname VARCHAR(255) NOT NULL, username VARCHAR(255) NOT NULL, pts VARCHAR(20) NOT NULL, date VARCHAR(50) NOT NULL, time VARCHAR(20) NOT NULL, ipaddr VARCHAR(255) NOT NULL, random VARCHAR(255) NOT NULL, PRIMARY KEY (random));" 2>/dev/null
        mysqlshow -h"$ipaddr" --user="$mysqluser" --password="$mysqlpass" status >> dbfile2.txt 2>/dev/null
...
rm dbfile.txt
#Edit Template;
userstemplate="/root/scripts/SECSUITE/inframon/node-install/users-monitor-local-store.template"
...
sed -i "s/host='MYHOSTHERE'/host='$ipaddr'/g" $userstemplate
...
ssh -p $rhostport $sudousr@$ipaddr "mv /root/scripts/SECSUITE/inframon/users-monitor-local-store.template /root/scripts/SECSUITE/inframon/users-monitor.sh"
sed -i "s/hostname='$hostname'/hostname='MYHOSTNAMEHERE'/g" $userstemplate
sed -i "s/host='$ipaddr'/host='MYHOSTHERE'/g" $userstemplate
...
while true; do
    ...
    case $yn in
        [Yy]* ) echo "Testing installed components..."
                ...
                sleep 2
                ssh -p $rhostport $sudousr@$ipaddr sudo bash /root/scripts/SECSUITE/inframon/users-monitor.sh
                echo ''
                ...
                mysql -h"$ipaddr" -D'status' --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM srv;" 2>/dev/null
                echo "Users:"
                mysql -h"$ipaddr" -D'status' --user="$mysqluser" --password="$mysqlpass" -e "SELECT hostname, username, pts, date, time, ipaddr, importtime AS Last_Scan FROM hist_loggedusers order by Last_Scan desc limit 2;" 2>/dev/null
                echo ''
...
#
nodeshower="show-all-probes-$hostname.sh"
echo '#!/bin/bash' >> $nodeshower
echo "ipaddr='$ipaddr'" >> $nodeshower
echo "mysqluser='$mysqluser'" >> $nodeshower
echo "mysqlpass='$mysqlpass'" >> $nodeshower
echo 'echo ""' >> $nodeshower
echo 'mysql -h"$ipaddr" -D"status" --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM apachestatus;" 2>/dev/null' >> $nodeshower
echo 'echo ""' >> $nodeshower
echo 'mysql -h"$ipaddr" -D"status" --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM cpu;" 2>/dev/null' >> $nodeshower
echo 'echo ""' >> $nodeshower
echo 'mysql -h"$ipaddr" -D"status" --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM srv;" 2>/dev/null' >> $nodeshower
echo 'echo ""' >> $nodeshower
echo 'mysql -h"$ipaddr" -D"status" --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM temperature;" 2>/dev/null' >> $nodeshower
echo 'echo ""' >> $nodeshower
echo 'mysql -h"$ipaddr" -D"status" --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM hist_loggedusers order by importtime desc limit 5;" 2>/dev/null' >> $nodeshower
echo 'echo ""' >> $nodeshower
noderunner="execute-all-probes-$hostname.sh"
echo '#!/bin/bash' >> $noderunner
echo "ipaddr='$ipaddr'" >> $noderunner
echo "sudousr='$sudousr'" >> $noderunner
echo "rhostport='$rhostport'" >> $noderunner
echo "hostname='$hostname'" >> $noderunner
echo 'ssh -p $rhostport $sudousr@$ipaddr bash /root/scripts/SECSUITE/inframon/apachestatus/apache-monitor-$hostname.sh' >> $noderunner
echo 'sleep 2' >> $noderunner
echo 'ssh -p $rhostport $sudousr@$ipaddr bash /root/scripts/SECSUITE/inframon/cpufiles/$hostname/load-avg-monitor-$hostname.sh' >> $noderunner
echo 'sleep 2' >> $noderunner
echo 'ssh -p $rhostport $sudousr@$ipaddr bash /root/scripts/SECSUITE/inframon/latency-files/$hostname-latency-monitor.sh' >> $noderunner
echo 'sleep 2' >> $noderunner
echo 'ssh -p $rhostport $sudousr@$ipaddr sudo bash /root/scripts/SECSUITE/inframon/users-monitor.sh' >> $noderunner
echo 'sleep 2' >> $noderunner
echo 'ssh -p $rhostport $sudousr@$ipaddr bash /root/scripts/SECSUITE/inframon/temperaturefiles/$hostname-temp-monitor.sh' >> $noderunner
#
exit
