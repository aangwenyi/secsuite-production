...
echo "You have chosen to install: " ; printf "${green}Remote Node Feedback${nc}\n"
echo ''
#
...
#  echo ''
#  printf "+ ${red} Your host Server IP addres has been found, and will require a Standard Local Installation.${nc}\n\n"
...
menu() {
  printf "Select a node by typing in the corresponding ${green} ID ${nc} number: "
  echo ""
  echo ""
  for (( i=1; i<${#menu[@]}; i++ )); do
...
echo 'fi' >> $installscript
#
read -p "Enter SSH User for Remote Node (root recommended): " sudousr
while true; do
...
read -p "Enter SSH User for Processing Server (root recommended): " procsudousr
while true; do
    read -p "Is $myip running SSH on port 22? (Y/n): " yn
    case $yn in
        [Yy]* ) procrhostport="22" ; break;;
        [Nn]* ) read -p "Please insert the SSH port number: " procrhostport ; break;;
        * ) echo "Please answer yes or no.";;
    esac
done
#Define IP
...
echo ''
echo ''
printf "${green}To Correctly Feedback the monitors, we will require a Remote MySQL User on this Processing Server:${nc}\n"
read -p "Remote MySQL User on this Processing Server: " procmysqluser
read -s -p "Please enter $mysqluser's MySQL Password on this Processing Server: " procmysqlpass
echo ''
#
...
echo "#" >> $constructfile
echo "mysqluser='$procmysqluser'" >> $constructfile
echo "mysqlpass='$procmysqlpass'" >> $constructfile
echo "host='$myip'" >> $constructfile
...
echo 'mysql -h $host --user=$ mysqluser --password=$ mysqlpass -e "use status;truncate table apachestatus;" 2>/dev/null' >> $constructfile
#
echo "Checking database for entries..."
mysqlshow -h $myip --user="$procmysqluser" --password="$procmysqlpass" status >> dbfile.txt 2>/dev/null
if grep -q apachestatus "dbfile.txt"; then
        ...
        mysql -h"$myip" -D'status' --user="$procmysqluser" --password="$procmysqlpass" -e "SELECT ID FROM apachestatus;" > dbidnums.txt 2>/dev/null
...
if ! grep -q apachestatus "dbfile.txt"; then
        ...
        mysql -h $myip -D'status' --user="$procmysqluser" --password="$procmysqlpass" -e "CREATE TABLE apachestatus (id INT AUTO_INCREMENT NOT NULL, hostname VARCHAR(255) NOT NULL, apachestatus VARCHAR(255) NOT NULL, PRIMARY KEY (id));" 2>/dev/null
...
if ! grep -q hist_apachestatus "dbfile.txt"; then
        ...
        mysql -h $myip -D'status' --user="$procmysqluser" --password="$procmysqlpass" -e "CREATE TABLE hist_apachestatus (id INT AUTO_INCREMENT NOT NULL, hostname VARCHAR(255) NOT NULL, apachestatus VARCHAR(255) NOT NULL, importtime VARCHAR(255) NOT NULL, primary key (importtime));"
        mysqlshow -h"$ipaddr" --user="$mysqluser" --password="$mysqlpass" status >> dbfile2.txt 2>/dev/null
...
echo "rsync -avzh -e 'ssh -p $procrhostport' /var/lib/mysql-files/importapachestatus.csv $procsudousr@$myip:/var/lib/mysql-files/ --quiet" >> $constructfile
echo 'mysql -h $host --user=$ mysqluser --password=$ mysqlpass -e "USE status;LOAD DATA INFILE ' >> qry1.txt
...
echo 'mysql -h $host --user=$ mysqluser --password=$ mysqlpass -e "use status;update select * from apachestatus;" 2>/dev/null' >> $constructfile
echo 'mysql -h $host --user=$ mysqluser --password=$ mysqlpass -e "use status;INSERT INTO hist_apachestatus SELECT *, CURRENT_TIMESTAMP() FROM apachestatus;" 2>/dev/null' >> $constructfile
echo "ssh -p $procrhostport $procsudousr@$myip 'rm /var/lib/mysql-files/importapachestatus.csv'" >> $constructfile
#echo 'mysql -h $host --user=$ mysqluser --password=$ mysqlpass -e "use status; SELECT * FROM hist_apachestatus ORDER BY importtime DESC LIMIT 1;" 2>/dev/null' >> $constructfile
...
#
while true; do
    read -p "Do you wish to use $procmysqluser as the MySQL user? (y/n): " yn
    case $yn in
...
#
    ...
    touch $cpumonitor
        ...
        echo "mysqluser='$procmysqluser'" >> $cpumonitor
        echo "mysqlpass='$procmysqlpass'" >> $cpumonitor
        echo "host='$myip'" >> $cpumonitor
        echo "#" >> $cpumonitor
        ...
        echo "mysql -h $ host --user=$ mysqluser --password=$ mysqlpass -e " >> qry1.txt
        ...
        echo " 2>/dev/null" >> qry1.txt
        tr -d '\n' < qry1.txt > qry2.txt
        echo "rsync -avzh -e 'ssh -p $procrhostport' /var/lib/mysql-files/cpuimport.csv $procsudousr@$ host:/var/lib/mysql-files/ --quiet" >> $cpumonitor
        cat qry2.txt >> $cpumonitor
        rm qry1.txt qry2.txt
        echo "" >> $cpumonitor
        echo 'mysql -h $ host --user=$ mysqluser --password=$ mysqlpass -e "use status;INSERT INTO hist_cpu SELECT *, CURRENT_TIMESTAMP() FROM cpu;" 2>/dev/null' >> $cpumonitor
        echo "ssh -p $procrhostport $procsudousr@$myip 'rm /var/lib/mysql-files/cpuimport.csv'" >> $constructfile
...
#Process the monitor
        ...
        sed -i 's/$ host/$host/g' $cpumonitor
...
mysql -h $myip -D'status' --user="$procmysqluser" --password="$procmysqlpass" -e "SELECT ID FROM cpu;" > dbidnums.txt 2>/dev/null
i=$(awk '/./{line=$0} END{print line}' dbidnums.txt)
...
if ! grep -q cpu "dbfile.txt"; then
        ...
        mysql -h $myip -D'status' --user="$procmysqluser" --password="$procmysqlpass" -e "CREATE TABLE IF NOT EXISTS cpu ( id INT AUTO_INCREMENT NOT NULL, hostname VARCHAR(255), loadonemin VARCHAR(10), loadtenmin VARCHAR(10), loadfifmin VARCHAR(10), x VARCHAR(10), y VARCHAR(10), PRIMARY KEY (id));" 2>/dev/null
fi
rm dbfile.txt
echo ''
#
...
echo "mysqlusername='$procmysqluser'" >> $constructfile
echo "mysqlpassword='$procmysqlpass'" >> $constructfile
echo "host='$myip'" >> $constructfile
...
echo "#" >> $constructfile
echo 'mysql -h $host --user=$mysqlusername --password=$mysqlpassword -e "use status;SELECT ID FROM cpu;" > dbidnums.txt 2>/dev/null' >> $constructfile
...
if ! grep -q srv "dbfile.txt"; then
        ...
        mysql -h $myip -D'status' --user="$mysqluser" --password="$mysqlpass" -e "CREATE TABLE IF NOT EXISTS srv ( id INT AUTO_INCREMENT NOT NULL, hostname VARCHAR(255), lastping VARCHAR(50), PRIMARY KEY (id));" 2>/dev/null
        mysqlshow -h $myip --user="$procmysqluser" --password="$procmysqlpass" status >> dbfile2.txt
...
#Get the ID numbers from the current database, add one, and use that as the ID for the new one.
mysql -h $myip -D status --user="$procmysqluser" --password="$procmysqlpass" -e "SELECT ID FROM srv;" > dbidnums.txt 2>/dev/null
i=$(awk '/./{line=$0} END{print line}' dbidnums.txt)
#Add one to make it acceptable
((i++))
#Import Web Server
mysql -h $myip -D status --user="$procmysqluser" --password="$procmysqlpass" -e "INSERT INTO srv (id, hostname, lastping) VALUES ('$i','$hostname Web Server','Awaiting Configuration');" 2>/dev/null
#Check
echo "Web Server Inserted with ID Number $i: "
mysql -h $myip -D status --user="$procmysqluser" --password="$procmysqlpass" -e "SELECT * FROM srv WHERE id = '$i';" 2>/dev/null
#Add another for the SSH Server
((i++))
#Import
mysql -h $myip -D status --user="$procmysqluser" --password="$procmysqlpass" -e "INSERT INTO srv (id, hostname, lastping) VALUES ('$i','$hostname SSH Server','Awaiting Configuration');" 2>/dev/null
#Check
...
#Get Current Amount of Configured Assets
mysql -h $myip -D status --user="$mysqluser" --password="$mysqlpass" -e "SELECT ID FROM srv;" > dbidnums.txt 2>/dev/null
i=$(awk '/./{line=$0} END{print line}' dbidnums.txt)
#
#Get All ID Numbers (smallest first, largest last)
cat dbidnums.txt | grep -oP '^[^0-9]*\K[0-9]+' > lastid.txt
...
#Import Web
mysql -h $myip -D status --user="$mysqluser" --password="$mysqlpass" -e "INSERT INTO srv (id, hostname, lastping) VALUES ('$filenum','$hostname Web Server','Awaiting Configuration');" 2>/dev/null
#
#SSH Maths:
one="1"
...
#Import SSH
mysql -h $myip -D status --user="$mysqluser" --password="$mysqlpass" -e "INSERT INTO srv (id, hostname, lastping) VALUES ('$filenumssh','$hostname SSH Server','Awaiting Configuration');" 2>/dev/null
#
...
echo "user='$procmysqluser'" >> $buildfile
echo "pass='$procmysqlpass'" >> $buildfile
echo "host='$myip'" >> $buildfile
echo "#" >> $buildfile
...
echo "mysql -h $ host --user=$ user --password=$ pass -e " >> qry1.txt
echo '"' >> qry1.txt
...
echo "  nmap $ hostdev -p$wsp > $ file01" >> $buildfile
echo "  lastping01=$ (cat $ file01 | grep 'latency')" >> $buildfile
echo "  mysql -h $ host --user=$ user --password=$ pass -e " >> qry2.txt
echo '"' >> qry2.txt
...
rm qry2.txt
echo "fi" >> $buildfile
echo 'mysql -h $ host --user=$ user --password=$ pass -e "USE status;INSERT INTO hist_srv (id, hostname, lastping, random, importtime) SELECT *, RAND(), CURRENT_TIMESTAMP() FROM srv;" 2>/dev/null' >> $buildfile
...
mysql -h $myip -D status --user="$procmysqluser" --password="$procmysqlpass" -e "SELECT * FROM srv;" 2>/dev/null
echo ''
...
if ! grep -q loggedusers "dbfile.txt"; then
        ...
        mysql -h $myip -D'status' --user="$mysqluser" --password="$mysqlpass" -e "CREATE TABLE loggedusers (hostname VARCHAR(255) NOT NULL, username VARCHAR(255) NOT NULL, pts VARCHAR(20) NOT NULL, date VARCHAR(50) NOT NULL, time VARCHAR(20) NOT NULL, ipaddr VARCHAR(255) NOT NULL, random VARCHAR(255) NOT NULL, PRIMARY KEY (random));" 2>/dev/null
        mysqlshow -h"$myip" --user="$procmysqluser" --password="$procmysqlpass" status >> dbfile2.txt 2>/dev/null
...
mysqlshow -h $myip --user="$procmysqluser" --password="$procmysqlpass" status >> dbfile.txt 2>/dev/null
if grep -q hist_loggedusers "dbfile.txt"; then
...
if ! grep -q hist_loggedusers "dbfile.txt"; then
        ...
        mysql -h $myip -D'status' --user="$procmysqluser" --password="$procmysqlpass" -e "CREATE TABLE hist_loggedusers (hostname VARCHAR(255) NOT NULL, username VARCHAR(255) NOT NULL, pts VARCHAR(20) NOT NULL, date VARCHAR(50) NOT NULL, time VARCHAR(20) NOT NULL, ipaddr VARCHAR(255) NOT NULL, importtime VARCHAR(255) NOT NULL, random VARCHAR(255) NOT NULL, PRIMARY KEY (random));" 2>/dev/null
        mysqlshow -h"$myip" --user="$procmysqluser" --password="$procmysqlpass" status >> dbfile2.txt 2>/dev/null
        if grep -q hist_loggedusers "dbfile2.txt"; then
        printf "${green} Table 'status.hist_loggedusers' has been created, continuing...${nc}\n"
        fi
        rm dbfile2.txt
fi
rm dbfile.txt
userstemplate="/root/scripts/SECSUITE/inframon/node-install/users-monitor-remote-store.template"
sed -i "s/mysqluser='MYMYSQLUSR'/mysqluser='$procmysqluser'/g" $userstemplate
sed -i "s/mysqlpass='MYMYSQLPASS'/mysqlpass='$procmysqlpass'/g" $userstemplate
sed -i "s/host='MYHOSTHERE'/host='$myip'/g" $userstemplate
sed -i "s/procrhostport='MYPORTHERE'/procrhostport='$procrhostport'/g" $userstemplate
sed -i "s/procsudousr='MYSUDOUSRHERE'/procsudousr='$procsudousr'/g" $userstemplate
sed -i "s/hostname='MYHOSTNAMEHERE'/hostname='$hostname'/g" $userstemplate
rsync -avzh -e "ssh -p $rhostport" $userstemplate $sudousr@$ipaddr:/root/scripts/SECSUITE/inframon/ --quiet
ssh -p $rhostport $sudousr@$ipaddr "mv /root/scripts/SECSUITE/inframon/users-monitor-remote-store.template /root/scripts/SECSUITE/inframon/users-monitor.sh"
sed -i "s/hostname='$hostname'/hostname='MYHOSTNAMEHERE'/g" $userstemplate
sed -i "s/procsudousr='$procsudousr'/procsudousr='MYSUDOUSRHERE'/g" $userstemplate
sed -i "s/procrhostport='$procrhostport'/procrhostport='MYPORTHERE'/g" $userstemplate
sed -i "s/mysqluser='$procmysqluser'/mysqluser='MYMYSQLUSR'/g" $userstemplate
sed -i "s/mysqlpass='$procmysqlpass'/mysqlpass='MYMYSQLPASS'/g" $userstemplate
...
while true; do
    ...
    case $yn in
        [Yy]* ) echo "Testing installed components..."
                ...
                echo ''
                echo "Apache Status:"
                mysql -h"$myip" -D'status' --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM apachestatus;" 2>/dev/null
                echo "CPU Load Averages:"
                mysql -h"$myip" -D'status' --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM cpu;" 2>/dev/null
                echo "CPU Temperatures:"
                ssh -p $rhostport $sudousr@$ipaddr bash /root/scripts/SECSUITE/inframon/temperaturefiles/$hostname-temp-monitor.sh
                echo "Latency:"
                mysql -h"$myip" -D'status' --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM srv;" 2>/dev/null
                echo "Users:"
                mysql -h"$myip" -D'status' --user="$mysqluser" --password="$mysqlpass" -e "SELECT * FROM loggedusers;" 2>/dev/null
                echo ''
                echo "If you would like to actively monitor $hostname, you will need to add the following lines in your crontab file on the Remote Node:"
